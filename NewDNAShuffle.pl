#!usr/bin/perl
# usage (mac): perl NewDNAShuffle.pl <inputFileName> <outputFileName>  <species one> <species two> <shufflecount> <windowsize> <shuffleINDELS(yn)> <TransAfter(y/n)>
...
sub shiftMutations {
  ...
  $shuffledSeq = shiftIndels($reference, $shuffledSeq, $windowSize);
	...
	# translate to protein if wanted
	if ($translateAfter eq "y"){
    ...
    /;
		...
		$shuffledSeq =~ tr/tagcT/UAGCU/;
		$translatedSeq = "";
		foreach my $codon ($shuffledSeq =~ /(...)/g) {
